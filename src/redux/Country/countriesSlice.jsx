...
const countryInformation = (data) => data.filter((country) => country.independent).map(
  (country) => ({
    ...
    map: `${COUNTRY_MAP_URL}${country.cca3.toLowerCase()}/vector.svg`,
    currencies: Object.entries(country.currencies).map(([code, {name, symbol}])  => ({code, name, symbol,})),
...
export const getCountries = createAsyncThunk( 
  ...
  async ({region , name}) => {
    ...
    const filteredCountries = response.data.filter((country) => country.continents[0] === name,
                                                  );
